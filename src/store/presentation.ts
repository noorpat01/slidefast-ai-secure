...
export const usePresentationStore = create<PresentationState>((set, get) => ({
  ...
  generatePresentation: async (params) => {
    ...
    // Create a timeout promise - extended for cost-effective AI processing
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Request timeout - Lightning-Fast AI Engine is taking longer than expected')), 75000) // 75 second timeout for Lightning-Fast AI Engine
    })
    
    try {
      // Race between the API call and timeout - Using Enhanced AI Generator
      const apiCall = supabase.functions.invoke('enhanced-ai-presentation-generator', {
        body: {
          topic: params.topic,
      ...
      if (!data || !data.data) {
        throw new Error('Invalid response from Enhanced AI service')
      }
      
      console.log('✅ Enhanced presentation generated successfully:', data.data.title)
      ...
      if (data.saved_id) {
        console.log('✅ Enhanced presentation auto-saved with ID:', data.saved_id)
        ...
        if (data.quality_feedback?.message) {
          ...
          toast.success('Enhanced presentation generated and saved successfully!')
        }
      } else {
        ...
        if (data.quality_feedback?.message) {
          ...
          toast.success('Enhanced presentation generated successfully!')
        }
      }
