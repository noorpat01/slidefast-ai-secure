Deno.serve(async (req) => {
    ...
    // Debug logging
    console.log(`📡 Website Router - URL: ${req.url}, pathname: ${pathname}, method: ${req.method}`);
    
    // Handle static assets by redirecting to the working deployment
    if (pathname.startsWith('/assets/') || pathname.endsWith('.js') || pathname.endsWith('.css') || pathname.endsWith('.png') || pathname.endsWith('.jpg') || pathname.endsWith('.svg') || pathname.endsWith('.ico') || pathname.endsWith('.woff') || pathname.endsWith('.woff2') || pathname.endsWith('.ttf') || pathname.includes('worker')) {
    ...
    // For all other requests, serve the React app HTML
    console.log(`🌐 Serving React app HTML for path: ${pathname}`);
